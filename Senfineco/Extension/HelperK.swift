...
class HelperK: NSObject {
    class func checkFirstTimeSetLocation()->Bool{
        let def = UserDefaults.standard
        return (def.object(forKey: "firstLocation") as? Int) != nil

    }
    class func setFirstLoc(first:Int){
        let def = UserDefaults.standard
        def.set(first, forKey: "firstLocation")
    }
    class func restarttApp() {
        var window: UIWindow? {
...
//        }
    ...
    class func saveCountryCode(country:String){
        let def = UserDefaults.standard
    ...
    class func openYoutube(youtube:String) {
        let youtubeUrl = URL(string:"youtube://\(youtube)")!
        ...
        if application.canOpenURL(youtubeUrl) {
            ...
            let webURL = URL(string: "https://www.youtube.com/watch?v=\(youtube)")!
